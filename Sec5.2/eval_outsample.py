...
def generate_perturbations(X_test_dataloader, pi_nets, inv_pcamatrix):
    ...
    for idx, batch_ in enumerate(test_source):
        ...
        gen_batch_10 = transport_cot(pi_nets[10], batch, 10)
        gen_samples[10] = torch.cat((gen_samples[10], gen_batch_10), 0)

        gen_batch_100 = transport_cot(pi_nets[100], batch, 100)
        gen_samples[100] = torch.cat((gen_samples[100], gen_batch_100), 0)

        gen_batch_1000 = transport_cot(pi_nets[1000], batch, 1000)
        gen_samples[1000] = torch.cat((gen_samples[1000], gen_batch_1000), 0)

        gen_batch_10000 = transport_cot(pi_nets[10000], batch, 10000)
...
def load_outsample_models():
    dosages = [10,100,1000,10000]
    models = dict.fromkeys(dosages)
    for each in dosages:
        models[each] = Generator(noise_dim=noise_dim)
        models[each] = models[each].to(device)
        models[each].load_state_dict(torch.load(f"outsample/model_{each}.pth"))
        models[each].eval()
...
def main():
    ...
    # pi_net.load_state_dict(torch.load("model_1.pth"))
    # pi_net.eval()
    pi_nets = load_outsample_models()
    adata = sc.read("./datasets/hvg.h5ad")
    # inv_pcamatrix = np.linalg.pinv(adata.obsm["X_pca"]) @ adata.X.A
    inv_pcamatrix = adata.varm["PCs"].T
    ...
    for trial in tqdm(range(num_repeats)):
        ...
        gen_samples = generate_perturbations(X_test_dl, pi_nets, inv_pcamatrix)
        gen_sample_save = {}
        for each in gen_samples.keys():
            gen_sample_save[each] = gen_samples[each].detach().cpu().numpy()

        perturbations.append(gen_sample_save)
        # compare l2
    ...
    with open(f'pickle_test_outsample.pkl','wb') as f:
        pickle.dump(perturbations, f)
    
    with open(f'pickle_metrics_outsample.pkl','wb') as f:
        pickle.dump([ps_dict, mmd_dict, ws_dict], f)
