...
from ot_cond.utils import set_seed, get_G, get_dist, freeze, unfreeze
import numpy as np
...
logger = createLogHandler("logs/correctness_exp_tabak.csv", str(getpid()))

parser.add_argument('--lambda_reg_x','-rx',default = 1000,type = float)
parser.add_argument('--lambda_reg_y','-ry',default = 1000,type = float)
...
def train_regr(dataloader, optimizer_T, optimizer_g, T, g, epochs, device = device, lambda_reg = lambda_reg_x, d=1, epochs_inner=5):
    ...
    for t in tqdm(range(epochs)):
        ...
        ws_dist = torch.trace(get_dist(test_y_gen, test_y.view(bs,-1)))/bs
        
        ep_obj.append(tot_obj)
        ws_obj.append(ws_dist.item())
        # torch.save(T.state_dict(), f'tabak_model/decay/T-{lambda_reg}-{args.lr}-{args.i_epochs}-{epochs}.pt')
        if(ws_dist<best_score):
            best_score = ws_dist
            best_model = T.state_dict()
    return ep_obj,ws_obj , T.load_state_dict(best_model), g
...
for testing_z in np.arange(0.1,1,0.1):
    ...
    testing_y = T(torch.cat([emperical_x.view(-1,1), testing_Z.repeat_interleave(n_noise).view(-1,1)], dim=1))

    tcost = torch.diagonal(get_dist(emperical_x.view(1,-1,d),testing_y.view(1,-1,d)),offset=0,dim1=-1, dim2=-2).sum()/(n_noise*n_ynoise)
    ...
    logger.info(f'{lambda_reg_x}, {lambda_reg_y}, {NOISE_DIMENSION}, {n_noise}; {n_ynoise}, {args.noise_var_x},{args.noise_var_y}, {lr}, {hidden_dim},\
        {n_epochs}, {khp_x}, {khp_y}, {n_X}, {batch_size}, {args.seed}, {testing_z}, {tcost}, {obj[-1]}, {ws_obj[-1]}, {wd_x} , {wd_y} \
            ')
